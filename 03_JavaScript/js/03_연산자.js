...
function 더하기() {
  //input 태그에 작성된 값 = value 값= value
  숫자1.value;
  숫자2.value;
  //숫자1.의 값과 숫자2.의 값을 더해줌
  /*
  ->input에서 type을 number로 설정해줘도 
  input 태그 안에서 작성된 값은 무조건 글자형식
  type=number는 가나다 ABC와 같은 문자작성을 못하게 막아주는 역할을 할 뿐
  
  [해결방법]
  문자->숫자로 변경하기를 해주면 됨
  ->컴퓨터 용어 데이터 파싱(data parsing) : 데이터의 종류를 바꿈(문자->숫자/ 숫자->문자)
  숫자만 작성된 문자를 진짜 숫자로 변경하는 방법 
  예) input 안에 숫자 123이 있다면 Number("123") 작성하면 됨 
...
/*나누기 연산 함수
나머지 연산(%)는 나누기 계산 시 몫이 정수인 부분까지만 계산하고 남은 나머지 값을 출력*/
function 몫() {
  const value1 = 숫자1.value;
  const value2 = 숫자2.value;
  결과.innerText = Number(value1) / Number(value2);
