 ...
 *  <button onclick="제출(event)">등록하기</button>
    위 버튼의 event = 사용자가 버튼이라는 행동을 감지했을 때 작동하는 설정 
    제출(매개변수) 매개변수에 어떤 동작이 들어왔는지 담는 변수명이기 때문에 
    굳이 위에 있는 이름과 맞춰서 작성할 의무 x 
 */
function 제출(e) {
  ...
  const emailRegExp = /^[a-zA-Z0-0.-_]+@[a-zA-Z0-0.-]+\.[a-zA-Z]{2,}$/;

  //유효성 검사
  //조건에 부합하지 않을 때만 if를 사용하고 싶음
  //1. 이름
  if (!nameRegExp.test(name)) {
    alert("이름이 정규식에 부합하지 않습니다.");
    return false;
  } //회원가입에 부합하지 않은 조건을 만날 때 돌려보내기
  //2. 나이
  if (!ageRegExp.test(age)) {
    ...
    //나이가 105이하일때만 회원가입 가능 , 106이상 작성시 가입하지 못하게 하고
    //alert로 "존재할 수 없는 나이입니다."
  }
  //3. 이메일
