function submitForm() {
  ...
  const marketingChecked = document.getElementById("marketing");

  //모든 필수 약관에 동의하지 않았다면 필수약관에 동의해주세요 알림
  // ******* !true= false      !false=true *********
  //if 안이 true가 되어야 alert 동작

  //!serviceChecked = 서비스 약관에 동의하지 않음 의미
  //!privacyChecked = 개인정보 보호 약관에 동의하지 않음을 의미
  //&& 모든 필수 약관에 체크를 안해야지 필수 약관 동의해달라는 표기가 보임
  // || 둘중 하나라도 필수 약관에 체크하지 않았다면 모두 체크하라는 표기가 보여질것
  // serviceChecked가 false인 경우 느낌표를 붙이면 true로 변경해줌
  //!serviceChecked = 서비스 항목에 체크 안했을때가 false 인데, 앞에 !를 붙여 true로 변한것
  if (!serviceChecked || !privacyChecked) {
    alert("모든 필수 약관에 동의해주세요.");
    return;
  }
  //선택된 취미 배열로 저장하는 for문 사용
  const hchecked = document.querySelectorAll(".hobby");
  let hobbies = "";
  let count = 0;
  for (let i = 0; i < hchecked.length; i++) {
    if (hchecked[i].checked) {
      hobbies += hchecked[i].value + " "; //선택한 취미 모두 이어붙이기
      count++; //취미 개수 증가
    }
  }
  //위 코드는 결과 표시를 보기 위한 진행중
  //결과 표시
